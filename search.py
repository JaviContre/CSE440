...
def uniformCostSearch(problem):
    ...
    while not priority_queue.isEmpty():
        ...
        if state not in visit:
            visit.add(state)
            successors = problem.getSuccessors(state)
            for succ, action, step in successors:
                total = cost + step
                priority_queue.push((succ, path + [action], total), total)
    return []
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not priority_queue.isEmpty():
        ...
        if state not in visit:
            ...
            for succ, action, step in successors:
                total = cost + step
                val = heuristic(succ, problem)
                priority_queue.push((succ, path + [action], total), total + val)
    return []
